...
//					System.out.println(dto);
			...
			//콘솔에 반드시 원인을 남겨야 한다
			printSqlError("글목록 조회(getAllList)", e);
		}
	...
	public boolean insertBoard(HkDto dto) {
		...
		String sql = " INSERT INTO HKBOARD(ID, TITLE, CONTENT, REGDATE) "
				   + " VALUES(?,?,?,SYSDATE()) ";
		...
		try(Connection conn=getConnection();
			...
			printSqlError("글 추가(insertBoard)", e);
		}
	...
	public HkDto getBoard(int seq){
		...
		try(Connection conn=getConnection();//2단계
			...
			printSqlError("글 상세보기(getBoard, seq=" + seq + ")", e);
		}
	...
	public boolean updateBoard(HkDto dto) {
		...
		try(Connection conn=getConnection();
			...
			printSqlError("글 수정(updateBoard, seq=" + dto.getSeq() + ")", e);
		}
	...
	public boolean deleteBoard(HkDto dto) {
		...
		try(Connection conn=getConnection();
			...
			printSqlError("글 삭제(deleteBoard, seq=" + dto.getSeq() + ")", e);
		}
	...
	public boolean mulDel(String[] seqs) {
		...
		try(Connection conn=getConnection();){
			...
			try(PreparedStatement psmt=conn.prepareStatement(sql)){
				...
				printSqlError("여러글 삭제 batch 실행(mulDel) - rollback 했음", e);
			}finally {
				//원래 설정으로 되돌리기
			...
			printSqlError("여러글 삭제 연결/트랜잭션 설정(mulDel)", e);
			isS=false;
		}
		return isS;
	}
