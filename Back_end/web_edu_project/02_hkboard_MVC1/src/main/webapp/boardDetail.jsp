<%@page import="com.hk.board.dto.HkDto"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>글 상세보기 및 수정</title>
</head>
<%
// requestScope에서 HkDto 객체 가져오기
HkDto dto = (HkDto) request.getAttribute("dto");

// 잘못된 접근(dto가 없는 경우) 시 안전하게 목록으로 튕겨내기
if (dto == null) {
%>
<script type="text/javascript">
		alert("조회할 게시글 데이터가 없습니다.");
		location.href = "boardController.jsp?command=boardlist";
	</script>
<%
	return;
}
%>
<body>
	<%-- 참고용 쿼리문은 JSP 주석으로 처리하여 화면 노출을 막습니다:
	     UPDATE HKBOARD SET TITLE=?, CONTENT=? WHERE SEQ=? --%>
	<h1>글 상세보기</h1>
	<form action="boardController.jsp" method="post">
		<!-- 컨트롤러 분기용 식별자 -->
		<input type="hidden" name="command" value="boardUpdate" />
		<!-- 수정할 대상 글 번호(PK) 전송 -->
		<input type="hidden" name="seq" value="<%=dto.getSeq()%>" />

		<table border="1">
			<tr>
				<th>작성자(ID)</th>
				<td><%=dto.getId()%></td>
			</tr>
			<tr>
				<th>글제목</th>
				<td><input type="text" name="title" value="<%=dto.getTitle()%>"
					required="required" style="width: 95%;" /></td>
			</tr>
			<tr>
				<th>글내용</th>
				<td>
					<%-- textarea는 태그 사이의 들여쓰기 공백까지 글 내용으로 들어가므로 한 줄로 밀착시킵니다 --%> <textarea
						rows="10" cols="60" name="content" required="required"><%=dto.getContent()%></textarea>
				</td>
			</tr>
			<tr>
				<td colspan="2" style="text-align: center;">
					<input
					type="submit" value="글수정" />
					<input type="button" value="글삭제"
					onclick="boardDelete(<%=dto.getSeq()%>)" />
					<input type="button" value="글목록"
					onclick="location.href='boardController.jsp?command=boardlist'" />
				</td>
			</tr>
		</table>
	</form>
	<script type="text/javascript">
		function boardDelete(seq){
			if(confirm("정말 삭제하겠습니까?")){
				location.href='boardController.jsp?'+'command=boardDelete&seq='+seq;
			}
		}
	</script>
</body>
</html>
