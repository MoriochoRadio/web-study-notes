...
[`verify-shelter-queries.sql`](verify-shelter-queries.sql)은 세션 임시 테이블만 만들어 ① `IS NULL`이 고르는 건수 ② `= NULL`이 0건이라는 것 ③ 빈 문자열이 `NULL`이 아니라는 것 ④ INNER JOIN이 짝 없는 행을 버린다는 것 ⑤ 뒤집힌 기록만 골라낸다는 것 ⑥ 정렬 순서를 확인하고, 마지막에 임시 테이블을 지운다. 수업 테이블이나 문제 원본 데이터는 건드리지 않는다.

> **이번 정리에서는 이 스크립트를 실행하지 않았다.** DB 접속 비밀번호가 이 환경에 없어 연결하지 못했다(`Access denied for user 'root'@'localhost'`). 위 SQL 설명은 손으로 따져 본 결과이고, 아래처럼 직접 돌려 확인할 수 있다.

```powershell
& "C:\Program Files\MariaDB 12.3\bin\mariadb.exe" -u root -p sqlDB < verify-shelter-queries.sql
```
