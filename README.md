...
두 대시보드 모두 **복습 탭**으로 열립니다 — "뭐부터 볼까"에 한 줄로 답하고 버튼 하나로 5분 플래시카드를 시작합니다(헷갈린다고 표시한 카드가 있으면 그것부터). 그 밖에 검색, 접기/펼치기, 다크 모드, 진도 체크, 오답 노트를 지원해 필요한 내용을 다시 찾는 시간을 줄입니다. 과목별 안내 문서에는 현재 범위와 폴더 구조를 기록하고, 수업 후에는 원본 코드와 복습 카드가 같은 단원을 가리키는지 함께 확인합니다.
