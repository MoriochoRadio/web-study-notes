...
과정을 기록합니다. 저는 기능 로직을 AI한테 대신 작성해달라고 하지 않고 직접 작성한
뒤 "이렇게 짰는데 리뷰해줘"라고 요청하는 방식으로 AI를 활용했습니다. 그래서 아래
사례들은 AI가 준 코드의 오류가 아니라, 제가 작성한 코드를 AI 리뷰와 실제 테스트로
검증하며 찾아낸 오류/한계를 분석하고 React 개념에 기반해 직접 고친 기록입니다.
실제로 겪은 사례는 이보다 많았지만, 그중 제가 직접 원인을 파악하고 고친 핵심 사례
6건만 골라 담았습니다.
...
이 사례는 AI가 준 코드에서 오류를 찾은 게 아니라, 제가 직접 짠 코드를 AI에게
리뷰받으며 원인만 설명 듣고 스스로 고친 과정입니다. 정답을 바로 받지 않고 매 라운드
"왜 틀렸는지"만 물어봤기 때문에 최종 코드는 전부 직접 작성한 셈입니다.
...
포인트로 "렌더링 최적화"를 명시했길래 그냥 넘기지 않고 짚었습니다.
...
눈으로 확인해야만 발견됩니다. 그래서 매 기능이 끝날 때뿐 아니라, 프로젝트를
