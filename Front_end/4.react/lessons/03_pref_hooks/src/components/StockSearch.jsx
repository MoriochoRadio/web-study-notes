...
export default function StockSearch() {
    ...
    const handleSearch = (e) => {
        ...
        setResults(
            q.trim()
                ? STOCKS.filter( //filter가 STOCKS 배열을 하나씩 흝으며 원소 하나 꺼낼때마다 (s) 실행 / 검사 함
                    // 화살표 함수 중괄호 {} 생략 ➔ 조건식 판단 결과(true/false)가 암시적으로 자동 return 됨
                    // 주식의 symbol 또는 name에 입력어 q가 들어있으면 true 반환 ➔ 검색 결과 배열에 포함!
                    (s) =>
                        s.symbol.toLowerCase().includes(q.toLowerCase()) ||
                        s.name.toLowerCase().includes(q.toLowerCase()) //filter로 하나씩 검사해서 조건에 맞는거만 뺴고 다 버리는거임
                )
                : [] // 검색어가 없거나 공백이면 빈 배열 반환
        ) //그래서 여기는 STOCKS 배열에서 입력값(q)을 포함하는 요소만 추려서 새로운 배열을 반환함
    }
